...
class LogManager:
    def __init__(self, file_path="logs_config.json"):
    ...
    def load_configs(self):
        ...
        if os.path.exists(self.file_path):
            try:
                ...
                print(f"✅ Configuration de logs chargée pour {len(self.log_configs)} serveur(s)")
    ...
    def set_log_channel(self, guild_id, channel_id):
        ...
        if guild_str not in self.log_configs:
            self.log_configs[guild_str] = {"channel_id": None, "enabled": False}
...
class LogFormatter:
    ...
    def create_log_embed(title, description, color, user=None, channel=None, extra_fields=None):
        ...
        if user:
            ...
            embed.add_field(name="👤 Utilisateur ID", value=user.id, inline=True)
